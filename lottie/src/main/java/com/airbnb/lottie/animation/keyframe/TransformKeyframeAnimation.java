...
public class TransformKeyframeAnimation {
  ...
  private BaseKeyframeAnimation<Point, Point> anchorPoint;
  private BaseKeyframeAnimation<?, Point> position;
  ...
  public <T> boolean applyValueCallback(T property,  LottieValueCallback<T> callback) {
    if (property == TRANSFORM_ANCHOR_POINT) {
      if (anchorPoint == null) {
        anchorPoint = new ValueCallbackKeyframeAnimation(callback, new Point());
      } else {
        anchorPoint.setValueCallback((LottieValueCallback<Point>) callback);
      }
    } else if (property == TRANSFORM_POSITION) {
      if (position == null) {
        position = new ValueCallbackKeyframeAnimation(callback, new Point());
      } else {
