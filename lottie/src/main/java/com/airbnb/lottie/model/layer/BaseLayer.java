...
public abstract class BaseLayer
  ...
  private final Paint contentPaint = new LPaint();
  private final Paint dstInPaint = new LPaint();
  private final Paint dstOutPaint = new LPaint();
  private final Paint mattePaint = new LPaint();
  ...
  BaseLayer(LottieDrawable lottieDrawable, Layer layerModel) {
    ...
    dstInPaint.setBlendMode(BlendMode.DST_IN);
    dstOutPaint.setAntiAlias(true);
    dstOutPaint.setBlendMode(BlendMode.DST_OUT);
    mattePaint.setAntiAlias(true);
  ...
  public void getBounds(
      RectFloat outBounds, Matrix parentMatrix, boolean applyParents) {
    rect.modify(0, 0, 0, 0);
  ...
  public void draw(Canvas canvas, Matrix parentMatrix, int parentAlpha) {
    ...
    if (!rect.getIntersectRect(new RectFloat(0, 0, canvas.getLocalClipBounds().getWidth(),
    ...
    if (rect.getWidth() >= 1f && rect.getHeight() >= 1f) {
      ...
      Utils.saveLayerCompat(canvas, rect, contentPaint, 0);
    ...
    if (outlineMasksAndMattes && outlineMasksAndMattesPaint != null) {
      ...
      outlineMasksAndMattesPaint.setColor(new Color(0xFFFC2803));
      ...
      outlineMasksAndMattesPaint.setColor(new Color(0x50EBEBEB));
      canvas.drawRect(rect, outlineMasksAndMattesPaint);
    }
  ...
  private void clearCanvas(Canvas canvas) {
    ...
    canvas.drawRect(new RectFloat(rect.left - 1, rect.top - 1, rect.right + 1, rect.bottom + 1), clearPaint);
  ...
  private void intersectBoundsWithMask(RectFloat rect, Matrix matrix) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      switch (mask.getMaskMode()) {
        ...
        default:
          ...
          if (i == 0) {
            maskBoundsRect.modify(tempMaskBoundsRect);
          } else {
            maskBoundsRect.modify(
    ...
    boolean intersects = rect.getIntersectRect(maskBoundsRect);
    if (!intersects) {
  ...
  private void intersectBoundsWithMatte(RectFloat rect, Matrix matrix) {
    ...
    boolean intersects = rect.getIntersectRect(matteBoundsRect);
    if (!intersects) {
      rect.modify(0f, 0f, 0f, 0f);
    }
  }
  ...
  private void applyInvertedSubtractMask(Canvas canvas, Matrix matrix, Mask mask,
      BaseKeyframeAnimation<ShapeData, Path> maskAnimation, BaseKeyframeAnimation<Integer, Integer> opacityAnimation) {
    Utils.saveLayerCompat(canvas, rect, dstOutPaint, 0);
    canvas.drawRect(rect, contentPaint);
  ...
  private void applyIntersectMask(Canvas canvas, Matrix matrix, Mask mask,
      BaseKeyframeAnimation<ShapeData, Path> maskAnimation, BaseKeyframeAnimation<Integer, Integer> opacityAnimation) {
    Utils.saveLayerCompat(canvas, rect, dstInPaint, 0);
    Path maskPath = maskAnimation.getValue();
    path.set(maskPath);
    path.transform(matrix);
    contentPaint.setAlpha((int) (opacityAnimation.getValue() * 2.55f));
